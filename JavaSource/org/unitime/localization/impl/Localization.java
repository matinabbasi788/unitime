...
public class Localization {
	...
	public static DateFormat getDateFormat(String pattern) { 
		if (getFirstLocale().startsWith("fa")) {
			// جلالی
			return new DateFormat() {
				...
				public StringBuffer format(Date date, StringBuffer toAppendTo, java.text.FieldPosition fieldPosition) {
					...
					LocalDate gregorian = date.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
					PersianDate persianDate = PersianDate.fromGregorian(gregorian);
					String formatted = persianDate.format(DateTimeFormatter.ofPattern(pattern));
					return toAppendTo.append(formatted);
				}
				@Override
				public Date parse(String source, java.text.ParsePosition pos) {
					try {
						PersianDate persianDate = PersianDate.parse(source, DateTimeFormatter.ofPattern(pattern));
						LocalDate gregorian = persianDate.toGregorian();
						Date date = Date.from(gregorian.atStartOfDay(ZoneId.systemDefault()).toInstant());
