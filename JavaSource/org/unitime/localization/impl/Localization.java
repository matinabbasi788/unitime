 ...
 * agreements. See the NOTICE file for additional information regarding copyright ownership.
 *
 * The Apereo Foundation licenses this file to you under the Apache License, Version 2.0
...
public class Localization {
    ...
    /** Adapter تاریخ جلالی */
    public static class PersianDateFormat extends DateFormat {
        private static final long serialVersionUID = 1L;
        private final SimpleDateFormat icuFormat;

        public PersianDateFormat(String pattern) {
            this.icuFormat = new SimpleDateFormat(pattern, new ULocale("fa_IR@calendar=persian"));
        }

        @Override
        public StringBuffer format(Date date, StringBuffer toAppendTo, FieldPosition pos) {
            return icuFormat.format(date, toAppendTo, pos);
        }
    ...
    public static DateFormat getDateFormat(String pattern) {
        if ("fa".equalsIgnoreCase(getJavaLocale().getLanguage())) {
            return new PersianDateFormat(pattern);
        } else {
            return new java.text.SimpleDateFormat(pattern, getJavaLocale());
        }
    }
    ...
    public static class Bundle implements InvocationHandler {
        ...
        public Bundle(Class<?> messages) { iMessages = messages; }

        private synchronized String getProperty(String locale, String name) {
            Properties properties = iProperties.get(locale);
            if (properties == null) {
                properties = new Properties();
                String resource = iMessages.getName().replace('.', '/') + 
                                  (locale.isEmpty() ? "" : "_" + locale) + ".properties"; 
                try {
                    InputStream is = Localization.class.getClassLoader().getResourceAsStream(resource);
                    if (is != null)
                        properties.load(is);
                } catch (Exception e) {
                    sLog.warn("Failed to load message bundle " + iMessages.getName() + " for " + locale + ": " + e.getMessage(), e);
                }
        ...
        private String[] string2array(String value) { return value.split("(?<=^.*[^\\\\]),(?=.*$)"); }
        ...
        private Object type(String value, Class returnType) {
            if (value == null) return value;
            if (String.class.equals(returnType)) return value;
            if (Boolean.class.equals(returnType) || boolean.class.equals(returnType)) return "true".equalsIgnoreCase(value);
            if (Double.class.equals(returnType) || double.class.equals(returnType)) return Double.valueOf(value);
            if (Float.class.equals(returnType) || float.class.equals(returnType)) return Float.valueOf(value);
            if (Integer.class.equals(returnType) || int.class.equals(returnType)) return Integer.valueOf(value);
            if (String[].class.equals(returnType)) return string2array(value);
            if (Map.class.equals(returnType)) {
                ...
                if (map.isEmpty()) return array2map(string2array(value));
                return map;
            }
            return value;
        }
        ...
        private Map<String, String> fixStringMap(Map<String, String> value, Map<String, String> defaults) {
            if (value != null && !value.keySet().equals(defaults.keySet())) {
                ...
                for (Map.Entry<String, String> e: defaults.entrySet()) {
                    if (!value.containsKey(e.getKey())) fixed.put(e.getKey(), e.getValue());
                }
                return fixed;
            }
            return value;
        ...
        public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
            ...
            if (value != null) {
                ...
                if (String[].class.equals(method.getReturnType())) {
                    ...
                    if (dsa != null) return fixStringArray((String[])ret, dsa.value());
                }
                if (Map.class.equals(method.getReturnType())) {
                    ...
                    if (dsm != null) return fixStringMap((Map<String, String>)ret, array2map(dsm.value()));
                }
                return ret;
            }
